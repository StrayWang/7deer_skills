---
name: youtube-demand-keywords
description: |
  YouTube 订阅频道热门需求关键词提取工具。自动抓取订阅频道视频，识别高潜力的 AI 工具、服务与应用需求关键词，
  归类为 AI 工具 / 出海服务 / 内容创作 等赛道，生成结构化日报。
  触发词：YouTube 需求关键词日报 / YouTube 订阅源 / 赛道热度 / YouTube demand keywords。
---

# YouTube Demand Keywords Skill

从 YouTube 订阅频道抓取需求相关内容，识别热门需求关键词并生成结构化日报。
...
本 skill 支持设置为每天自动执行。在 Hermes 中创建以下 cron job 即可：

**创建命令：**
```
hermes cron add \
  --name "YouTube Demand Keywords 日报" \
  --schedule "0 10 * * *" \
  --tz "Asia/Shanghai" \
  --payload-kind agentTurn \
  --payload-message "使用 youtube-demand-keywords skill，执行 YouTube 订阅频道需求关键词日报任务。按照 skill 完整流程执行：1. 先尝试运行官方脚本（超时则降级用 browser 抓 YouTube 订阅页）2. 整理需求关键词日报 3. 发送飞书消息到飞书用户 4. 创建飞书云文档并写入完整报告 5. 在飞书消息里附上文档链接" \
  --payload-timeout 900 \
...
```json
{
  ...
  "payload": {
    "kind": "agentTurn",
    "message": "使用 youtube-demand-keywords skill，执行 YouTube 订阅频道需求关键词日报任务。按照 skill 完整流程执行：1. 先尝试运行官方脚本（超时则降级用 browser 抓 YouTube 订阅页）2. 整理需求关键词日报 3. 发送飞书消息到飞书用户 4. 创建飞书云文档并写入完整报告 5. 在飞书消息里附上文档链接",
    "timeoutSeconds": 900
  },
  "delivery": {
    "mode": "announce",
...
Hermes 的 browser 和你的本地浏览器**共享同一个浏览器 profile**，所以：

1. 你在 Chrome/Safari/Edge 里登录了 YouTube
2. Hermes browser 打开订阅页就自动带着你的登录态
...
# 启动 Hermes browser 使用指定 profile
hermes browser start --profile ~/Library/Application\ Support/Google/Chrome/Default

# 然后在浏览器里手动登录 YouTube（只需一次）
```

**方法二：启动独立的 Chrome profile 供 Hermes 专用**

```bash
# 1. 创建新的 Chrome profile 目录
mkdir -p ~/hermes-browser-profile

# 2. 启动 Hermes browser 指定该 profile
hermes browser start --profile ~/hermes-browser-profile
...
| 问题 | 原因 | 解决 |
|------|------|------|
| 页面显示"登录 YouTube" | Browser 没有 YouTube 登录态 | 手动在 Hermes browser 里登录 YouTube |
...
```
youtube-demand-keywords/
├── SKILL.md                        # Skill 标准格式（本文件）
├── README.md                       # 使用说明（含 Python 脚本模式）
└── scripts/
    ├── fetch_subscriptions.sh       # Hermes Agent 抓取脚本
...
> ⚠️ **重要**：执行此步前必须确认 Hermes browser 已登录 YouTube（见上方配置说明）。
...
| 分类 | 说明 |
|------|------|
| AI 工具 | AI 工具、模型、工作流相关内容 |
| 出海服务 | 支付、物流、广告、合规等出海配套服务 |
| 内容创作 | YouTube, TikTok, Blog 等内容运营技巧与需求 |
| 商业模式 | 搞钱案例、SaaS 模型、Indie Hacker 经验 |
| 其他 | 不属于以上类别 |
...
【值得关注的爆发点】（播放量 >50K 的非主流高增长需求）
| 需求 | 频道 | 播放 | 类型 | 亮点 |

【算法流量猎奇发现】（高播放量但细分冷门的领域）
| 领域 | 频道 | 播放 | 发现点 |
```
...
> ⚠️ 独立运行同样需要在运行前确保 `hermes` CLI 已登录 YouTube（在 browser 里登录一次即可）。

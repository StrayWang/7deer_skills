...
# 通过 Hermes agent 抓取 YouTube 订阅频道视频
# 用法: ./fetch_subscriptions.sh [输出JSON路径]
#
# 依赖:
#   - hermes CLI 已安装并配置
#   - 已在终端登录过 YouTube（Hermes browser 使用同一浏览器 session）
#
# 示例:
...
OUTPUT="${1:-./data/reports/hermes_subscriptions_videos.json}"

PROMPT='Use the browser tool to inspect https://www.youtube.com/feed/subscriptions in the current Hermes browser session. Scroll down several times to load more items (at least 3-5 scrolls). Return strict JSON with this exact structure:
{
  "logged_in": boolean,
...
response=$(hermes agent --agent main --message "$PROMPT" --json 2>&1)

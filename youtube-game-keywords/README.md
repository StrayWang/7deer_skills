# YouTube Demand Keywords — Hermes Skill 版

本 skill 有两种使用方式：

1. **Hermes Agent 模式**（推荐）：Agent 读 SKILL.md 自动执行全流程
2. **Python 脚本模式**：直接运行 `analyze_keywords.py`（抓取仍需 hermes CLI）

## 文件说明

| 文件 | 说明 |
|------|------|
| `SKILL.md` | Hermes Skill 标准格式（供 Agent 调用） |
| `README.md` | 本文档（含 Python 脚本模式说明） |
| `scripts/fetch_subscriptions.sh` | 通过 Hermes Agent 抓取 YouTube 订阅（需 hermes CLI） |
...
Hermes browser 和你的本地浏览器共享同一个 Chrome profile.
...
hermes browser open "https://www.youtube.com/feed/subscriptions"
```

### ⏰ 定时任务配置

本 skill 支持每天自动执行。在 Hermes 中运行以下命令即可启用定时：

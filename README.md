...
每个技能都不是凭空设计的 — 它们来自真实的独立站运营、AI 工具与服务站搭建和出海内容项目，经过反复打磨后整理成可复用模块。
...
🚀 **AI 服务站批量生产** — 做 AI 工具与服务聚合站最头疼的是内容更新。`multi-ai-service-hub` (原 multi-game-codes-hub) 让你 5 分钟从一个 JSON 文件生成完整的工具/服务展示页面（含分类分区、一键复制/跳转、FAQ Schema）；`ai-service-data-scraper` (原 roblox-game-data-scraper) 直接从 Trello 看板、Discord 频道和社交平台抓取最新情报，不用手动搬运。
...
| **Hermes** | `~/.hermes/skills/` 或项目内 `.agent/skills/` | 自动加载，HermesHub 分发 |
...
# Hermes（全局安装，所有项目共享）
git clone https://github.com/kennyzir/7deer_skills.git ~/.hermes/skills/7deer
...
| 5 | **youtube-content-gen** | YouTube 内容生成器（视频转 SEO 页面） | AI 工具/教程类站点 |
| 6 | **youtube-demand-keywords** | YouTube 订阅频道热门需求关键词提取 | 内容创作/AI 赛道 |
...
| # | 技能名称 | 描述 | 适用场景 |
|---|---------|------|---------|
| 8 | **ai-service-data-scraper** | Trello/Discord/Reddit 最新服务数据抓取 | AI 服务/工具网站 |
...
| # | 技能名称 | 描述 | 适用场景 |
|---|---------|------|---------|
| 17 | **multi-ai-service-hub** | 快速生成 AI 工具/服务页面（模板 + 组件） | AI 工具/服务网站 |
| 18 | **rpg-stat-catalyst** | 核心数值计算引擎（属性加点 + 阈值） | 复杂计算类应用 |
| 19 | **ai-site-architect** | AI 工具/服务站 SEO 架构规范 | AI 站/工具站 |
...
```bash
# 1. 准备数据文件
cat > ai_tools.json << EOF
{
  "categoryName": "AI Writing Tools",
  "categorySlug": "ai-writing",
  "activeTools": [
    {"name": "Jasper", "desc": "Enterprise AI content platform"}
  ]
}
EOF
...
python .agent/skills/multi-game-codes-hub/resources/generate_code_page.py \
  --input ai_tools.json \
  --output ./src/app/ai-writing/page.tsx
...
# 抓取看板数据（如最新的 AI 工具发布清单）
data = scraper.scrape_board('ai-tools-launch-board-id')
...
# 分类搜索意图
intent = classify_intent("best ai logo generator for small business")
...
│
├── multi-game-codes-hub/              # 🚀 (Rebranded: multi-ai-service-hub) AI 服务聚合器
│   ├── SKILL.md
│   ├── USAGE.md
│   └── resources/
...
│       └── examples/
│
├── roblox-game-data-scraper/          # 🚀 (Rebranded: ai-service-data-scraper) 服务情报抓取器
│   ├── SKILL.md
│   └── resources/
│       └── trello_scraper.py
│
...
├── youtube-game-keywords/             # 🆕 (Rebranded: youtube-demand-keywords) 需求词提取
...
├── roblox-site-architect/             # 🆕 (Rebranded: ai-site-architect) AI 服务站架构
...
### 2026-04-07
- 添加 **youtube-demand-keywords**（原 youtube-game-keywords，YouTube 热门需求关键词提取）

### 2026-04-04
- 🔥 添加 **google-trends-to-pages**（Google Trends → SEO 页面生成器）
- 🔥 添加 **multi-ai-service-hub**（原 multi-game-codes-hub，5 分钟生成 AI 工具/服务聚合页）
- 🔥 添加 **ai-service-data-scraper**（原 roblox-game-data-scraper，服务情报数据抓取）
...
- 添加 **ai-site-architect**（原 roblox-site-architect，AI 服务站架构）

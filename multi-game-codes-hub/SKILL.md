---
name: multi-ai-service-hub
description: 快速为任何 AI 工具或服务生成完整的聚合页面。包含分类展示、多语言支持、数据追踪、FAQ Schema 和自动更新机制。
keywords: ai tools, ai services, tool list, service aggregator, multilingual
---

# Multi-AI Service Hub - AI 工具与服务聚合中心

这个 skill 帮助你在 5 分钟内为任何 AI 赛道创建一个完整的、SEO 优化的工具展示页面。

## 核心价值主张

聚合页面是 AI 工具与服务网站的最高流量入口。这个 skill 提供了一个统一的模板系统，让你可以快速复制成功的工具展示结构到新赛道。
...
1. **自动更新检测** - 监控 Product Hunt/Twitter 新工具上线
2. **服务可用性验证** - 自动测试服务链接是否仍然有效
3. **性价比分析** - 自动计算不同服务的价格区间
4. **社区贡献** - 允许用户提交新发现的工具
5. **版本记录** - 追踪工具的功能迭代和 API 更新时间线
## 页面结构
...
```bash
# 1. 准备工具数据 (JSON 格式)
cat > ai_writing_tools.json << EOF
{
  "categoryName": "AI Writing Assistants",
  "categorySlug": "ai-writing",
  "activeTools": [
    {
      "name": "Jasper AI",
      "desc": "Enterprise-level AI writing platform",
      "launchDate": "2021-01-01"
    },
    {
      "name": "Copy.ai",
      "desc": "AI marketing copy generator",

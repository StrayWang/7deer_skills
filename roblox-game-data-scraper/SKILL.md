---
name: ai-service-data-scraper
description: 自动化抓取 AI 工具与服务情报数据的完整工具链。支持从 Product Hunt、Indie Hackers、Twitter 和 Trello 收集功能、价格、技术栈等结构化数据。
keywords: ai tools, data scraping, product hunt, indie hackers, automation
---

# AI Service Data Scraper - 服务情报自动化抓取器

这个 skill 帮助你自动化收集全球 AI 工具与服务的所有核心数据，是内容生成的数据基础。

## 核心价值主张

手动从 Product Hunt、Indie Hackers、Twitter 收集工具数据需要数小时。这个 skill 能在 10 分钟内自动抓取并结构化所有数据，直接生成可用的 TypeScript 类型定义。
...
interface AITool {
  ...
  launchDate: string;
  source: 'Product Hunt' | 'Twitter' | 'Indie Hackers';
  verifiedDate: string;
}
